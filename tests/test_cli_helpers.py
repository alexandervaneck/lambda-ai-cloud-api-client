...
def test_parse_tags_success_and_invalid(monkeypatch):
...
def test_cmd_list_other_resources(monkeypatch, capsys):
    outputs = []
    monkeypatch.setattr(cli, "_build_client", lambda args: "CLIENT")
    ...
    def fake_resp(name):
        def _inner(**kwargs):
            outputs.append(name)
            return DummyResponse(200, parsed={name: True})

        return _inner

    monkeypatch.setattr(cli, "list_instance_types", fake_resp("instance_types"))
    monkeypatch.setattr(cli, "list_images", fake_resp("images"))
    monkeypatch.setattr(cli, "list_ssh_keys", fake_resp("ssh_keys"))
    ...
    cli._cmd_list_ssh_keys(argparse.Namespace(token=None, base_url="https://example", insecure=False))

    assert outputs == ["instance_types", "images", "ssh_keys"]
    out = capsys.readouterr().out
    assert '"instance_types": true' in out
    assert '"images": true' in out
    assert '"ssh_keys": true' in out
